import{a as s}from"../src/chunk-OFWT6L5I.js";import"../src/chunk-PAEI7YB2.js";import"../src/chunk-5VY55ISN.js";var c=14,m=36,b=`
    <style>
        :root {
...
`,f=()=>{document.querySelector("body > template").content.querySelectorAll("ld-module[name='embedded-iframe']").forEach(a=>{let e=a.textContent;e=e.replace("{{ld-embedded-iframe.head.frag.html}}",b),e=e.replace("{{ld-embedded-iframe.editable-styles.frag.html}}",h),a.innerHTML=e})};s.addEventListener("beforeLDDOMManipulations",f);var i={},o=(t,a)=>{let e=i[t];if(!e){console.error(`iframe ${t} not found`);return}if(e.height)console.log(`not scaling iframe ${t} because it has an explicit height: ${e.height}`);else{let n=e.contentWindow.document.documentElement.getBoundingClientRect().height+"px";console.log(`scaling iframe ${t}: ${e.dataset.title}; reason = ${a}; new height = ${n}`),e.style.height=n}};window.addEventListener("message",t=>{let a=t.data["ld-iframe-applied-font-size"];a&&requestAnimationFrame(()=>{o(a,"iframe changed base font size")})});function g(){document.querySelectorAll("iframe.embedded-iframe, ld-module[name='embedded-iframe'] > iframe").forEach((t,a)=>{let e=a+1;i[e]=t,t.dataset.ldIframeId=e;let d=getComputedStyle(t).getPropertyValue("--current-base-font-size");t.style.boxSizing="content-box",t.addEventListener("load",()=>{t.contentWindow.postMessage({"ld-presentation-id":lectureDoc2.presentation.id,"ld-iframe-id":e,"ld-effective-font-size":d},"*"),setTimeout(()=>new IntersectionObserver((n,r)=>{n.forEach(l=>{l.isIntersecting&&(o(e,"intersection"),r.disconnect(),t.closest("ld-section")&&new ResizeObserver(()=>{o(e,"resize")}).observe(t))})}).observe(t))})})}s.addEventListener("afterLDDOMManipulations",g);

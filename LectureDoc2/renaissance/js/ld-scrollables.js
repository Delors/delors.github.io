 ...
 * an explicit height. When we find a previous state, we will scroll it to the
 * last state.
 * 
...
function initializeScrollable(scrollable) {
    ...
    if (scrollable.dataset.height && !scrollable.dataset.height.startsWith("-")) {
        ...
        if (scrollable.dataset.height && scrollable.dataset.height.startsWith("-")) {
            const height = "calc(" + baseHeight + "px + " + scrollable.dataset.height + ")";
    ...
    if (scrollable.closest("#ld-slides-pane")) {
        ...
        if (state) {
            ...
            if (scrollProgress) {
                scrollable.scrollTo({ top: scrollProgress, behavior: "instant" });
            }
        }
    }
}
...
function afterLDListenerRegistrations() {
    ...
    document.querySelectorAll("#ld-slides-pane ld-scrollable").forEach((scrollable,id) => {
        scrollable.dataset.scrollableId = id;
        scrollable.addEventListener("scroll", () => {
            // the state object may have been updated with a saved state
            const scrollProgress = lectureDoc2.getState()["slideScrollProgress"];
    ...
    if (ephemeral.ldPerDocumentChannel /* recall: no document id - no channel */) {
        ...
        document.querySelectorAll("#ld-slides-pane ld-scrollable").forEach((scrollable) => {
            ...
            ld.addScrollingEventListener(
                channel, "scrollableScrolled", 
                scrollable, scrollable.dataset.scrollableId);
        });
    }
...
lectureDoc2.ldEvents.addEventListener("afterLDDOMManipulations", afterLDDOMManipulations);
lectureDoc2.ldEvents.addEventListener("afterLDListenerRegistrations", afterLDListenerRegistrations);
